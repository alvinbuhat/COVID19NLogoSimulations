...
;  ]
  ...
  [
    ...
    ifelse (pycor < 0 and pxcor > 2) or (pycor < 0 and pxcor < 1) or (pycor > 1 and pxcor > 2) or (pycor > 1 and pxcor < 1)  ;train
    [set pcolor 46]
    [set pcolor 86]
  ...
  [
    ...
    ifelse social-distancing? [
      ifelse who < 12
      [
      move-to one-of patches with [ pcolor = 46 and
          ( pxcor = 8 or pxcor = 6 or pxcor = 4 or pxcor = 2 or
            pxcor = -5 or pxcor = -3 or pxcor = -1 or pxcor = 1)
          and not any? turtles-here ;and not any? turtles in-radius 1
      ]]

      [
        move-to one-of patches with [ pcolor = 86 and not any? turtles in-radius 1 ]]
    ]
    [
      ifelse who < 24
      [
...
"Exposed" 1.0 0 -1184463 true "plot count turtles with [pcolor = green and color = yellow]" "plot count turtles with [ color = yellow]"
...
<experiments>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="crowd-density" first="0" step="0.1" last="2"/>
    <enumeratedValueSet variable="social-distancing?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="maximum-capacity">
      <value value="100"/>
    </enumeratedValueSet>
